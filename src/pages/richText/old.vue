...
function withStyleOverride(content: string) {
  ...
  const styledContent = /font-family\s*:/i.test(content)
    ? content.replace(/font-family:[^;]+;?/i, nextFont)
    : `<div style="${nextFont}">${content}</div>`
...
function normalizeHtml(content: string) {
  ...
  const needsDecode = /&(?:lt|gt|amp;lt|amp;gt|quot|#39|amp;quot|amp;#39);/i.test(content)
  return needsDecode ? decodeHtmlEntities(content) : content
}
...
watchEffect(async () => {
  ...
  try {
    ...
    const remoteFamily = res.family?.trim() || REMOTE_FONT_FAMILY

    responseFamily.value = remoteFamily
...
<template>
  <view class="rich-text-page" size="full" p="4">
    <RichTextHeader v-model:value="richTextInput" />
